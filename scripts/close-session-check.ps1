...
$archiveChanged = $changedFiles | Where-Object {
  $_ -match "^\s*(M|A|\?\?)\s+(VOCABULARY\.md|DECISIONS\.md|BUGS\.md|WORKFLOW\.md|README\.md|SESSION_NOTES\.md)" -or
  $_ -match "^\s*(M|A|\?\?)\s+ai-skills/" -or
  $_ -match "^\s*(M|A|\?\?)\s+ai-prompts/"
}
...
if ([string]::IsNullOrWhiteSpace($VerificationCommand)) {
  Write-CheckLine "Verification" "not provided; record the build/test command in SESSION_NOTES.md"
} else {
...
Write-Output "3. Update SESSION_NOTES.md"
Write-Output "4. Commit"
Write-Output "5. Push"
Write-Output "6. Handoff"
